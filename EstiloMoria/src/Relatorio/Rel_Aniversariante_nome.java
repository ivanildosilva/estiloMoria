 ...
 */
public class Rel_Aniversariante_nome {
    conexao con_rel = new conexao();
    /** Creates a new instance of Rel_Cliente */
    public Rel_Aniversariante_nome() {
        ...
        try {
            ...
            con_rel.executeSQL("Select c.nm_cliente, dt_dia_nascimento,dt_mes_nascimento, t.cd_tel1, t.cd_tel2, e.nm_email1, en.nm_logradouro, en.cd_numero, en.cd_cep,en.nm_cidade  FROM Cliente c Left Join Telefone t ON (c.cd_cliente=t.cd_cliente) left join email e on (c.cd_cliente=e.cd_cliente) left join endereco en on (c.cd_cliente=en.cd_cliente) where nm_cliente LIKE  '"
                    + ConsultaAniversario.txtnome.getText().toUpperCase() + "%' order by dt_dia_nascimento");
            JRResultSetDataSource jrRS = new JRResultSetDataSource(con_rel.resultset);
            JasperPrint jasperPrint  = JasperFillManager.fillReport("C:/Projeto Netbeans/EstiloMoria/relatorios/Rel_Aniversario_nome.jasper", new HashMap(), jrRS);
